...
export function DayTabs({ setChangeDate }) {
  ...
  let todays_data = TaskEntries.filter(
    (task) => task.date === d.toDateString()
  );
  let todays_timers;
  let todays_total;
  if (todays_data.length !== 0) {
    todays_timers = todays_data.map((el) => Number(el.timer));
    todays_total = todays_timers.reduce((ac, el) => {
      return ac + el;
    });
  }
  ...
  if (TaskEntries.length !== 0) {
    ...
    total = total_timers.reduce((ac, el) => {
      return ac + el;
    });
  }
  ...
  return (
    <div>
      <div className={style.showdate}>
        <button onClick={() => changeDate(-1)} style={{ width: "40px" }}>
          <i class="fa fa-angle-left" style={{ fontSize: "15px" }}></i>
        </button>
        <button onClick={() => changeDate(1)} style={{ width: "40px" }}>
          <i class="fa fa-angle-right" style={{ fontSize: "15px" }}></i>
        </button>
        <h2>
          <span>{Prev === 0 && "Today: "}</span>
          {d.toDateString()}
        </h2>
      ...
      <div className={style.maindiv}>
        ...
        {!taskloading && todays_data.length !== 0 && (
          <div className={style.total_timer}>
            <h1>
              Total: <span>{todays_total}:00</span>
            </h1>
          </div>
        )}
        ...
        {!taskloading && todays_data.length === 0 && (
          <div className={style.container}>
            ...
            <div className={style.main_img}>
              <img
                src="https://cache.harvestapp.com/assets/onboarding/landing-timesheets-day@2x-ba92c8a231563661a31ba7af8e20e4dd1b19e6286b94065e7a45aec949752ce6.png"
                alt="img"
              />
            </div>
            <div className={style.ready}>
              ...
              <Link>
                <h2>Got it! I don’t need this tip anymore.</h2>
              </Link>
            </div>
          </div>
        )}
